                                    ...
                                    @forelse ($tblOrders as $order)
                                        <tr>
                                            ...
                                            @if ($order->order_status == 'order-placed')
                                                <td data-label="Payment"><label class="order-paid">Order placed</label></td>
                                            @elseif ($order->order_status == 'picked up')
                                                <td data-label="Payment"><label class="order-pending">Picked up</label></td>
                                            @else
                                                <td data-label="Payment"><label class="order-progress">In Progress</label>
                                                </td>
                                            @endif
                                            <td data-label="Date">{{ explode(' ', $order->created_at)[0] }}</td>
                                            <td data-label="Time">{{ explode(' ', $order->created_at)[1] }}</td>
                                            <td data-label="Action">
                                                <a href="{{ route('vendor_order_details', ['id' => $order->id]) }}"
                                                    class="orders-view-btn">
                                                    View
                                                </a>
                                                @if ($vendors->vendor == 'pharmacy')
                                                /
                                                <a href="{{ route('order_status_update', ['id' => $order->order_id]) }}"
                                                    class="orders-view-btn">
                                                    Picked Up
                                                </a>
                                                @endif
