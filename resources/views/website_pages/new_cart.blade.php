                        ...
                        $('#loadItemChecoutFinal').append('<div class="card mb-3">'+
                                '<div class="row g-0">'+
                                    '<div class="d-flex">'+
                                        ...
                                        '</div>'+
                                        '<div class="card-body">'+
                                            '<h5 class="card-title">'+product.product_image+'</h5>'+
                                            '<p class="card-text">Qty:1 <span class="float-end pe-3"><b> Price: $'+product.update_price+'</b></span></p>'+
                                                ...
                                                @forelse ($forCart['cartItems'] as $item)
                                                    @if($item['show_product']==1)
                                                        <li class="item_{{ $counter }}" style="background-color:#08295a;">
                                                            <input type="hidden" id="cartitemid_{{ $counter }}" value="{{ $item['id'] }}">
                                                            <div class="row">
                                                                <div class="row main align-items-center" style="position: relative">
                                                                    <div class="col-2 cart-img-div">
                                                                        <img class="img-fluid" alt="{{ $item['name'] }}" src="{{ url('/uploads/' . $item['product_image']) }}"/>
                                                                    ...
                                                                    <div class="col">
                                                                        <div class="row heading_{{ $counter }}" style="color:white;">{{ $item['name'] }}</div>
                                                                        @if ($item['item_type'] == 'prescribed')
                                                                            @if (!empty($item['medicine_usage']) && $item['product_mode'] != 'lab-test')
                                                                            <div class="row text-prescribed">{{ $item['medicine_usage'] }}</div>
                                                                            @endif
                                                                            @isset($item['prescribed_by'])
                                                                                <div class="row text-prescribed">Priscribed: Dr.{{ $item['prescribed_by'] }} - {{  Carbon\Carbon::parse($item['prescription_date'])->format('m/d/Y h:i A'); }}</div>
                                                                            @endisset
                                                                        @endif
                                                                        @if ($item['product_mode'] == 'lab-test' && $item['item_type'] == 'counter')
                                                                            <div class="row text-prescribed"> +$6.00 Provider's Fee</div>
                                                                        @endif
                                                                    </div>
                                                                    ...
                                                                    <div class="col price_{{ $counter }}" style="color: white;">${{ number_format($item['price'], 2) }}
                                                                        <span class="close">
                                                                            ...
                                                                            @if ($item['item_type'] == 'counter')
                                                                                ...
                                                                                <a href="{{ route('remove_item_from_cart',['id'=>$item['id']]) }}"><i class="fa-solid fa-trash-can text-danger"></i></a>
                                                                                <!-- <a href="#" class="cross quick-fix remove_cart_items" product_id={{ $item['product_id'] }} cart_row_id={{ $item['id'] }} style="color:grey"><i class="fa fa-trash"></i> Remove</a> -->
                                                                            @endif
                                                                        </span>
                                                                    </div>
                                                                </div>
                                                        ...
                                                        <li class="item_{{ $counter }}">
                                                            ...
                                                            <div class="row">
                                                                <div class="row main align-items-center" style="position: relative">
                                                                    ...
                                                                    <div class="col">
                                                                        <div class="row heading_{{ $counter }}">{{ $item['name'] }}</div>
                                                                        @if ($item['item_type'] == 'prescribed')
                                                                            @if (!empty($item['medicine_usage']) && $item['product_mode'] != 'lab-test')
                                                                            <div class="row text-prescribed">{{ $item['medicine_usage'] }}</div>
                                                                            @endif
                                        ...
                                        <div class="payment-order-summary-wrap">
                                            <div class="card">
                                                ...
                                                <ul class="list-group list-group-flush">
                                                    <li class="list-group-item">Total Item <span id="totalItem">{{ count($forCart['cartItems']) }}</span></li>
                                        ...
                                        <div class="card">
                                            ...
                                            <form method="post" action="{{ route('order.done') }}">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="text" value="baqir raza" name="card_holder_name" class="form-control mt-1" id="exampleInputEmail1" placeholder="Haris Unar" aria-describedby="emailHelp"/>
                                                </div>
                                                <div class="mb-3">
                                                    <label for="exampleInputPassword1" class="form-label">Card Number</label>
                                                    <div id="emailHelp" class="form-text">Enter the 16 digit card number on the card</div>
                                                    <div class="input-group">
                                                        <span class="input-group-text p-0" style="width: 10%; background-color: #c0d1dc38">
                                                            <img src="https://download.logo.wine/logo/Mastercard/Mastercard-Logo.wine.png" alt="" width="100%"/>
                                                        </span>
                                                        <input type="text" name="card_number" class="form-control" value="5499740000000057" placeholder="1234 - 2345 - 3445 - 2324"/>
                                                    </div>
                                                </div>
                                                <div class="row mb-3 align-items-center">
                                                    <div class="col-md-6">
                                                        <label for="inputPassword6" class="form-label">CVV Number</label>
                                                        <p id="passwordHelpInline" class="form-text">Enter the 4 or 5 digit numbers on card</p>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <input type="number" name="cvc" value="998" id="inputPassword6" class="form-control" aria-describedby="passwordHelpInline" />
                                                    </div>
                                                </div>
                                                <div class="row align-items-center mb-3">
                                                    <div class="col-md-6">
                                                        <label for="inputPassword6" class="form-label">Expiry Date</label>
                                                        <p id="passwordHelpInline" class="form-text">Enter the expiration date of thne card</p>
                                                    </div>
                                                    <div class="row col-md-6">
                                                        <div class="col-md-5">
                                                            <input type="number" value="12" name="exp_month" id="inputPassword6" class="form-control" aria-describedby="passwordHelpInline" />
                                                        </div>
                                                        <div class="col-md-2 text-center m-auto" style="font-size: 2rem">/</div>
                                                        <div class="col-md-5">
                                                            <input type="number" value="2022" name="exp_year" id="inputPassword6" class="form-control" aria-describedby="passwordHelpInline"/>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col-md-4 mb-3">
                                                        ...
                                                        <input type="number" name="zipcode" value="35242" class="form-control mt-3" id="zipcode" placeholder="Zip Code" aria-describedby="emailHelp"/>
                                                    </div>
                                                    <div class="col-md-4 mb-3">
                                                        <label for="state_code" class="form-label">State</label>
                                                        <input type="text" value="AL" name="state_code" class="form-control mt-3" id="state_code" placeholder="State" aria-describedby="emailHelp"/>
                                                    </div>
                                                    <div class="col-md-4 mb-3">
                                                        <label for="city" class="form-label">City</label>
                                                        <input type="text" value="Huntsville" name="city" class="form-control mt-3" id="city" placeholder="City" aria-describedby="emailHelp"/>
                                                    </div>
                                                </div>
                                                <div class="mb-3">
                                                    <label for="exampleInputEmail1" class="form-label">Address</label>
                                                    <input type="text"  value="abc flat 3 road none" name="address" class="form-control" id="exampleInputEmail1" placeholder="Address" aria-describedby="emailHelp" />
                                                </div>
