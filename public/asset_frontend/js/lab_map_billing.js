$(document).ready(function () {
    $(".billingLabZipCodeBtn").click(function (e) {
        e.preventDefault();

        let zipCode = $(".billingLabZipCodeInput").val();

        if (zipCode.length == 0) {
            alert("Please Enter zipcode.");
        } else {
            $(".show_map_1 #map3").show();
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (ress === 0) {
                        ...
                        $(".selectLabNearbyLocation").html("");
                        $(".selectLabNearbyLocation").append(
                            `<option value="">Select Your Neareast Lab Location</option>`
                        );
                            ...
                            success: function (res) {
                                ...
                                downloadUrl(test, function (data) {
                                    ...
                                    Array.prototype.forEach.call(
                                        markers,
                                        function (markerElem) {
                                            var id = markerElem.getAttribute(
                                                "id"
                                            );
                                            var name = markerElem.getAttribute(
                                                "name"
                                            );
                                            var address = markerElem.getAttribute(
                                                "address"
                                            );
                                            var type = markerElem.getAttribute(
                                                "type"
                                            );
