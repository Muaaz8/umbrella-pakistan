...
class unAuthController extends Controller
{
    ...
    public function searchPharmacyItemBySubId(Request $request,$vendor){
        $products = DB::table('tbl_products')
            ->join('products_sub_categories', 'products_sub_categories.id', '=', 'tbl_products.sub_category')
            ->join('vendor_products', 'vendor_products.product_id', '=', 'tbl_products.id')
            ->join('vendor_accounts', 'vendor_accounts.id', '=', 'vendor_products.vendor_id')
            ...
            ->where('vendor_products.is_active', 1);
            if($request->sub_id != 'all'){
                $products = $products->where('tbl_products.sub_category', $request->sub_id);
            }
            ...
            ->paginate(12)
            ->withPath('/pharmacy') // 👈 change pagination base path
            ->appends([
                'id' => $vendor,
